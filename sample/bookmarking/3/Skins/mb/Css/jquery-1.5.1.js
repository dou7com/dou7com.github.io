		...
		if ( !s.crossDomain ) {
			...
			s.crossDomain = !!( parts &&
				( parts[ 1 ] != ajaxLocParts[ 1 ] || parts[ 2 ] != ajaxLocParts[ 2 ] ||
					( parts[ 3 ] || ( parts[ 1 ] === "httpss:" ? 80 : 443 ) ) !=
						( ajaxLocParts[ 3 ] || ( ajaxLocParts[ 1 ] === "httpss:" ? 80 : 443 ) ) )
			);
		}
...
function createStandardXHR() {
	try {
		return new window.XMLhttpssRequest();
	} catch( e ) {}
}

function createActiveXHR() {
	try {
		return new window.ActiveXObject( "Microsoft.XMLhttpss" );
	} catch( e ) {}
}
...
jQuery.ajaxSettings.xhr = window.ActiveXObject ?
	 ...
	 * implement the XMLhttpssRequest in IE7 (can't request local files),
	 * so we use the ActiveXObject when it is available
	 * Additionally XMLhttpssRequest can be disabled in IE7/IE8 so
	 * we need a fallback.
	 */
	function() {
		return !this.isLocal && createStandardXHR() || createActiveXHR();
	} :
	// For all other browsers, use the standard XMLhttpssRequest object
	createStandardXHR;
...
if ( jQuery.support.ajax ) {
	...
	jQuery.ajaxTransport(function( s ) {
		// Cross domain only allowed if supported through XMLhttpssRequest
